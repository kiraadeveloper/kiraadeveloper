<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,100:1e293b&height=200&section=header&text=Jorge%20Santos%20|%20FullStack%20Developer&fontSize=35&fontColor=ffffff&animation=fadeIn"/>

# 👨‍💻 Jorge Santos | FullStack Dev

🎓 Estudante de Ciência da Computação  
🐍 Desenvolvedor focado em Python, sistemas e automação  
💻 Construindo projetos reais e soluções práticas  
🚀 Criador de conteúdo tech compartilhando evolução na programação  

---

## 🧠 Sobre mim
Sou apaixonado por transformar ideias em sistemas funcionais.
Gosto de desenvolver projetos envolvendo lógica, automações,
interfaces gráficas e aplicações completas.

Sempre evoluindo através de novos desafios,
testando tecnologias e fortalecendo minha base técnica.

---

## ⚙️ Tecnologias

### 🎨 Front-end
![HTML](https://img.shields.io/badge/HTML5-111827?style=for-the-badge&logo=html5)
![CSS](https://img.shields.io/badge/CSS3-111827?style=for-the-badge&logo=css3)
![JavaScript](https://img.shields.io/badge/JavaScript-111827?style=for-the-badge&logo=javascript)

### 🧠 Back-end
![Python](https://img.shields.io/badge/Python-111827?style=for-the-badge&logo=python)
![Java](https://img.shields.io/badge/Java-111827?style=for-the-badge&logo=openjdk)
![Git](https://img.shields.io/badge/Git-111827?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-111827?style=for-the-badge&logo=github)

### 🧩 Ferramentas
![Tkinter](https://img.shields.io/badge/Tkinter-111827?style=for-the-badge)
![Pandas](https://img.shields.io/badge/Pandas-111827?style=for-the-badge&logo=pandas)
![SQLite](https://img.shields.io/badge/SQLite-111827?style=for-the-badge&logo=sqlite)

---

## 📦 Projetos em Destaque
🔹 **Sistema de Estoque em Python**  
Interface gráfica, geração de relatórios em PDF e visualização de dados.

🔹 **Gerador de Senhas**  
Aplicação para criação de senhas seguras e personalizadas.

🔹 **Jogo Jokenpô com Interface**  
Projeto focado em lógica, estrutura e GUI.

---

## 📈 Atualmente Estudando
- Automação com Python
- Arquitetura de sistemas
- Banco de dados
- Boas práticas de desenvolvimento

---

## 📫 Contato
📧 Email: vjorgematheus0@email.com    
📸 Instagram: https://instagram.com/kiraadev_

---

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:1e293b,100:0f172a&height=120&section=footer"/>
